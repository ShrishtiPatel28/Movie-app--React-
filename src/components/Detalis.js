  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <button
          onClick={goToPreviousMovie}
          className="text-zinc-300 hover:text-zinc-100 absolute top-[45%] left-[5%] z-[900] rounded-full border border-zinc-700 hover:border-zinc-600 px-3 py-2 sm:left-[1.2%]"
        >
          <i className="ri-arrow-left-double-line text-xl"></i>
        </button>
        <button
          onClick={goToNextMovie}
          className="text-zinc-300 hover:text-zinc-100 absolute top-[45%] right-[5%] z-[900] rounded-full border border-zinc-700 px-3 py-2 sm:right-[0.3%]"
        >
          <i className="ri-arrow-right-double-line text-xl"></i>
        </button>
      </div>

      <div className="w-full sm:w-[92%] h-auto absolute left-1/2 transform -translate-x-1/2 sm:left-[50px] mt-16 sm:mt-28">
        {/* Movie Details Section */}
        <div className="movie-details flex flex-col sm:flex-row p-4 gap-10 sm:gap-16 font-bold text-sm sm:text-base">
          <img
            src={movieDetails.Images}
            alt={movieDetails.title}
            className="w-full sm:w-[300px] h-[440px] object-cover opacity-85 border border-slate-300 p-4"
          />
          <div className="text-white sm:w-[60%]">
            <h1 className="text-3xl sm:text-6xl font-extrabold mt-5 sm:mt-0">
              {movieDetails.title}
              <sub className="text-sm sm:text-lg text-zinc-200">
                ({movieDetails.Year})
              </sub>
            </h1>

            <div className="nav1 flex flex-wrap justify-start items-center gap-7 mt-5">
              <div className="italic text-lg w-16 h-16 text-center py-3 px-2 bg-yellow-500 rounded-full opacity-90 text-[17px] font-bold">
                {movieDetails.imdbRating}
                <sup className="font-bold">%</sup>
              </div>
              <p className="text-xs sm:text-sm capitalize font-normal">
                <i className="ri-calendar-fill text-sm text-yellow-500 mr-[3px]"></i>
                {movieDetails.Released}
              </p>
              <p className="text-xs sm:text-sm capitalize font-normal italic">
                <i className="ri-timer-line text-sm text-yellow-500 mr-[3px]"></i>
                {movieDetails.Runtime}
              </p>
              <p className="text-xs sm:text-sm capitalize font-normal">
                <i className="ri-play-circle-fill text-sm text-yellow-500 mr-[3px]"></i>
            ...
            <p className="text-sm sm:text-base capitalize font-normal text-white mt-4 italic">
              {movieDetails.Genre}
            </p>

            {/* Movie Overview */}
            <div className="overview w-full sm:w-[600px] mt-4">
              <h1 className="text-2xl font-normal">Overview</h1>
              <p className="text-sm sm:text-base font-normal mt-2 italic leading-6 tracking-wide text-zinc-300">
                {movieDetails.Plot}
              </p>
            </div>

            <p className="text-xl sm:text-[18px] capitalize font-normal text-white mt-5">
              Movie Translated -{" "}
              <p className="text-sm sm:text-base font-normal mt-2 italic leading-6 tracking-wide text-zinc-300">
                English, Spanish, French, German, Italian, Chinese, Japanese,
                Russian, Arabic, Portuguese, Hindi, Korean, Dutch, Turkish,
            ...
            <div className="btn mt-4">
              <button
                onClick={toggleTrailer}
                className="text-white font-extrabold bg-yellow-500 px-6 py-2 rounded"
              >
                {isTrailerPlaying ? "Playing" : "Watch Trailer"}
              </button>
              ...
              {isTrailerPlaying && (
                <div className="fixed inset-0 bg-black bg-opacity-70 w-full h-full flex justify-center items-center z-50">
                  <div className="relative">
                    <button
                      onClick={closeTrailer}
                      className="absolute top-0 right-0 text-white font-extrabold bg-red-600 px-6 py-2 rounded-full"
                    >
                      <i className="text-white ri-close-large-fill"></i>
                    </button>
                    <iframe
                      width="800"
                      height="450"
                      ...
                      allow="accelerometer; autoplay; muted; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                      allowFullScreen
                      className="w-full sm:w-[800px] sm:h-[450px] rounded"
                    ></iframe>
                  </div>
                </div>
              )}
            ...
            {/* Movie Cast, Writer, and Director */}
            <div className="mt-4">
              <p className="text-lg sm:text-xl capitalize font-normal italic text-white mt-4">
                Cast -{" "}
                <span className="text-sm sm:text-base capitalize font-normal text-zinc-300">
                  {movieDetails.Actors}
                </span>
              </p>
              <p className="text-lg sm:text-xl capitalize font-normal italic text-white mt-4">
        ...
        <hr className="border-none h-[1px] bg-zinc-200 mt-6" />
